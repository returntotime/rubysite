...
class ProductImport < ActiveRecord::Base
  ...
  validates_attachment_content_type :data_file, :content_type => ["text/csv", "image/gif"]
  ...
  def import_data!
    ...
    #  log("Names of products before import #{@names_of_products_before_import}")
      ...
      rows[IMPORT_PRODUCT_SETTINGS[:rows_to_skip]..-1].each do |row|
        ...
        logger.info "Prod Info #{product_information}"
...
#        if product = Spree::Product.where(sku: product_information[:sku]).exists?
#          IMPORT_PRODUCT_SETTINGS[:image_fields].each do |field|
#               find_and_attach_image_to(product, product_information[field.to_sym])
#               if product.save
#		 logger.warn "Images uploaded for existing"
#               else
...
#        end
        ...
        log("IMport product settings #{IMPORT_PRODUCT_SETTINGS[:create_variants]}")

        if IMPORT_PRODUCT_SETTINGS[:create_variants]
          field = IMPORT_PRODUCT_SETTINGS[:variant_comparator_field].to_s
          Rails.logger.info "Field in import data #{field}"
  ...
  def fetch_local_image(filename)
    ...
    unless File.exists?(filename) && File.readable?(filename)
      ...
      logger.warn "Image #{filename} was not found on the server, so this image was not imported."
      return nil
    else
      log("Image #{filename} was uploaded. Found. HKJSHKLJKLSJIOJSIOJSDO:JDL:KJDKL:JDKLJDLJDKLJDKLJDUIOJWLJWKLJIOJDJLDKJKLDJDJDIO")
      logger.warn "Image #{filename} was uploaded. Found. HKJSHKLJKLSJIOJSIOJSDO:JDL:KJDKL:JDKLJDLJDKLJDKLJDUIOJWLJWKLJIOJDJLDKJKLDJDJDIO"
