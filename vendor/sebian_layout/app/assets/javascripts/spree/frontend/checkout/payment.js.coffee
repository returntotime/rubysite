Spree.ready ($) ->
  Spree.onPayment = () ->
    if ($ '#checkout_form_payment').is('*')
      ...
      Spree.handlePlaceOrderAcceptance()
      ...
      ($ 'input[type="radio"][name="order[payments_attributes][][payment_method_id]"]').click(->
        window.current_payment_method_id = @value
...
#      Spree.handlePlaceOrderClicking()
...
#    $payment_method =($ '#payment_method_' + window.current_payment_method_id)
#    if $payment_method.data("type") != undefined && $payment_method.data("type") == "paypal-express"
#      $payment_method.click()
#      false

Spree.handlePlaceOrder = () ->
  $("#place-order").on "click", ->
    if $("#payment-method-fields input[type='radio']:checked").data('type') == 'paypal'
      $("#paypal_button").click()
    else
      $('#checkout_form_payment').submit()

Spree.handlePlaceOrderAcceptance = () ->
  $("#accept-place-order").on "change", ->
    if $(this).is(":checked")
