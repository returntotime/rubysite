...
class Spree::BlogEntry < ActiveRecord::Base
  ...
  scope :show_on_homepage, -> { where :show_on_homepage => true }
  scope :recent, lambda{|max=5| visible.limit(max) }
  scope :lastest, -> { order(:created_at => "desc").limit(3) }
  ...
  has_one :blog_entry_banner, :as => :viewable, :dependent => :destroy, :class_name => 'Spree::BlogEntryBanner'
  has_one :blog_entry_large_banner, :as => :viewable, :dependent => :destroy, :class_name => 'Spree::BlogEntryLargeBanner'
  accepts_nested_attributes_for :blog_entry_image, :reject_if => :all_blank
  accepts_nested_attributes_for :blog_entry_banner, :reject_if => :all_blank
  accepts_nested_attributes_for :blog_entry_large_banner, :reject_if => :all_blank
  ...
  def display_author
    unless self.author.blank? or self.author.nickname.blank?
