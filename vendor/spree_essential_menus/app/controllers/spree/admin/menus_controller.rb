class Spree::Admin::MenusController < Spree::Admin::ResourceController
  ...
  # before_filter :get_name_taxon, :only => :index
  before_filter :get_taxons, :only => [:new, :edit, :update]
  ...
  private
    ...
    def get_taxons
      @taxons = Spree::Taxon.all.order(:name)
    end
    ...
    def get_name_taxon
      if params[:taxon_id].present?
        @taxons ||= Spree::Menu.find(:all, :include => :taxon)
      end
    end
